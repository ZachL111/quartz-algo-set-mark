...
`quartz-algo-set-mark` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for set analysis with bounded scenario files, conflict explanations, and documented operating limits.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 6.
